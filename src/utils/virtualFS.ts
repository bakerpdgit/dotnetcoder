...
/**
 * Make sure the filesystem has at least one source file for `language`, so the
 * editor always has something to show after a language switch. Returns the path
 * of the file to open, or null when the filesystem already had sources.
 */
export async function ensureLanguageEntryPoint(fsId: string, language: LanguageId): Promise<string | null> {
  const lang = LANGUAGES[language]
  // A hidden language cannot be selected or run, so seeding a starter file for
  // it would only litter the filesystem with a file nobody can do anything with.
  if (lang.hidden) return null
...
/**
 * Every source file in `fsId` that belongs to `language`, sorted by path so a
 * compilation is deterministic. This is what gets handed to Roslyn — the whole
 * filesystem is the project, so students can split code across files.
 */
export async function getSourceFiles(fsId: string, language: LanguageId): Promise<Array<{ path: string; text: string }>> {
  ...
  // F# compiles in file order and requires the entry point to come last, so
  // move it there. C# and VB.NET are order-independent.
  if (language === 'fsharp') {
    const isEntryPoint = (file: { path: string; text: string }) =>
      file.text.includes('[<EntryPoint>]') || /\/Program\.fsx?$/i.test(file.path)
    return [...files.filter(f => !isEntryPoint(f)), ...files.filter(isEntryPoint)]
  }
...
export function guessMimeType(filename: string): string {
  ...
  const map: Record<string, string> = {
    cs: 'text/x-csharp', vb: 'text/x-vb', fs: 'text/x-fsharp', fsx: 'text/x-fsharp',
    csproj: 'application/xml', vbproj: 'application/xml', fsproj: 'application/xml',
    sln: 'text/plain', config: 'application/xml', xml: 'application/xml',
