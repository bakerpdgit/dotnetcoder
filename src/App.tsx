...
import {
  loadActiveFilesystem, loadFixedInput, loadLanguage, loadLayout, loadRunArgs, loadTheme,
...
export function App() {
  ...
  useEffect(() => {
    ...
    // Runs once: later changes go through switchLanguage / switchFilesystem.
  ...
  // ── Language / filesystem switching ─────────────────────────────────────

  const switchLanguage = useCallback(async (next: LanguageId) => {
    ...
    try {
      ...
      if (target) {
        setCurrentPath('/')
        await openPath(activeFilesystemId, target)
      }
      setReloadTrigger(t => t + 1)
      if (getLanguage(next).experimental) {
        showBanner(`${getLanguage(next).label} support is experimental — see the README.`)
      }
    } catch (error) {
  ...
  const switchFilesystem = useCallback(async (id: string) => {
    ...
    setReloadTrigger(t => t + 1)
  }, [flushSave, language, openPath, showBanner])
    ...
    switch (runner.phase) {
      case 'loading-runtime':
        return runner.statusDetail
          ? `loading the .NET runtime — ${runner.statusDetail}`
          : 'loading the .NET runtime…'
      case 'loading-references': return `loading the class library… ${runner.statusDetail}`
  ...
  return (
    <div className="flex h-screen w-screen flex-col gap-2 bg-slate-950 p-2">
      <header className="flex shrink-0 flex-wrap items-center gap-2 rounded-xl border border-slate-700 bg-slate-900/60 px-3 py-2">
        {/* Descriptive, not a brand: the MIT licence on the .NET runtime grants
            no trademark rights, so the name says what this is rather than
            leading with Microsoft's mark. See THIRD-PARTY-NOTICES.md. */}
        <h1 className="mr-2 whitespace-nowrap text-sm font-semibold tracking-tight text-slate-100">
          A .NET <span className="text-emerald-400">Coder</span>
        </h1>
        ...
        <label className="flex items-center gap-1.5 text-xs text-slate-400">
          ...
          <select
            ...
            onChange={(event) => {
              ...
              if (isLanguageId(next)) void switchLanguage(next)
            }}
            className="rounded-md border border-slate-600 bg-slate-800 px-2 py-1 text-sm text-slate-100 outline-none focus:border-emerald-500"
          >
            {SELECTABLE_LANGUAGES.map(entry => (
              <option key={entry.id} value={entry.id}>
                {entry.label}{entry.experimental ? ' (experimental)' : ''}
              </option>
            ))}
          </select>
        </label>
        ...
        {/* WebAssembly cannot be interrupted from outside, so stopping means
            discarding the worker and booting a fresh runtime. The same button
            doubles as a retry when the runtime failed to start. */}
        <button
          type="button"
          onClick={runner.stop}
          disabled={!runner.running && runner.ready}
          title={runner.running
            ? 'Stop the program and restart the .NET runtime'
            : 'Restart the .NET runtime'}
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <IconButton label="About, limitations and credits" onClick={() => setAboutOpen(true)}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
