...
createServer((req, res) => {
  ...
  if (!existsSync(filePath) || statSync(filePath).isDirectory()) {
    ...
    // place of a missing asset turns "the .NET runtime bundle is not built" into
    // a JSON parse error somewhere far away, which is a horrible thing to debug.
    if (extname(filePath)) {
...
}).listen(PORT, () => console.log(`dotnetcoder serving dist/ on http://localhost:${PORT}`))
